...
const FirstSection = () => {
    ...
    const img4Path = "src/Components/LandingPage/img4.jpg";
    ...
    return (
        <section className="bg-gradient-to-r from-white to-yellow-100 text-center pb-10 pl-5 pr-5 md:pl-10 md:pr-10 lg:pl-20 lg:pr-20">
            <div className="flex justify-between items-center w-full py-4 px-5 md:px-10 lg:px-20">
                ...
                {/* Account Section */}
                <div className="flex items-center space-x-4 md:space-x-6 cursor-pointer" onClick={handleOpenPopup}>
                    ...
                    {isPopupOpen && <LoginPopup isOpen={isPopupOpen} onClose={handleClosePopup}/>}
                </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center text-center mt-6 md:mt-0">
                ...
                <div className="flex flex-col lg:flex-row items-center justify-between mt-6 md:mt-8 max-w-6xl mx-auto">
                    ...
                    <div className="lg:hidden mb-4 p-3 md:p-5">
                        <img src={img4Path} alt={t.imgAltFindRoommate as string} className="mx-auto" style={{ width: '90%', maxWidth: '500px'}} />
                    </div>
                    {/* Text Content */}
                    <div className="max-w-md md:max-w-2xl">
                        <div className="bg-yellow-100 p-4 rounded-lg shadow-2xl m-3 md:m-5 text-left transition-transform duration-300 hover:scale-105">
                            <div className="p-2">
                                <p className="text-lg md:text-xl font-normal text-red-600">
                                    {t.feature1}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="hidden lg:block mt-4 lg:mt-0">
                        <img src={img4Path} alt={t.imgAltFindRoommate as string} className="mx-auto mb-4 p-3 md:p-5" style={{ width: '500px'}} />
                    </div>
                </div>
            </div>
        </section>
...
const HowItWorks = () => {
    ...
    const img3Path = "src/Components/LandingPage/img3.jpg";
        ...
        <section className="bg-gradient-to-r from-white to-yellow-100 text-center py-8 md:py-10 pl-5 pr-5 md:pl-0 md:pr-0 lg:pl-10 lg:pr-10">
            ...
            <div className="mt-4 flex flex-col lg:flex-row items-center justify-center gap-6">
                ...
                <div className="lg:hidden mb-4">
                    <img src={img3Path} alt={t.imgAltCreateProfile as string} className="mx-auto" style={{ width: '90%', maxWidth: '310px'  }} />
                </div>
                {/* Image for Normal Mode */}
                <div className="hidden lg:block lg:order-1">
                    <img src={img3Path} alt={t.imgAltCreateProfile as string} className="mx-auto mb-4" style={{ width: '310px'  }} />
                </div>
                {/* Steps */}
    ...
    return (
        <section className="py-12 md:py-16 text-center bg-gradient-to-r from-white to-yellow-100">
            ...
            <div className="mt-4 flex flex-col md:flex-row justify-center items-stretch gap-4 md:gap-6 max-w-6xl mx-auto lg:gap-8">
                {/* Adjusted width for 3 columns */}
                <div className="bg-yellow-100 p-6 md:p-10 rounded-lg shadow-2xl w-full md:w-1/3 transition-transform duration-300 hover:scale-105 flex flex-col">
                ...
                <div className="bg-white p-6 md:p-10 rounded-lg shadow-2xl w-full md:w-1/3 bg-gradient-to-r from-yellow-100 to-white transition-transform duration-300 hover:scale-105 flex flex-col">
                ...
                <div className="bg-white p-6 md:p-10 rounded-lg shadow-2xl w-full md:w-1/3 transition-transform duration-300 hover:scale-105 flex flex-col">
...
const LandingPage = () => {
    ...
    // State initializer function to read from localStorage only once
    const [language, setLanguage] = useState<'en' | 'fr'>(() => {
        try {
            const savedLanguage = localStorage.getItem('preferredLanguage');
            if (savedLanguage === 'en' || savedLanguage === 'fr') return savedLanguage;
        } catch (error) { console.error("LS Read Error:", error); }
        return 'en'; // Default
    });
    ...
    useEffect(() => {
        ...
        } catch (error) { console.error("LS Write Error:", error); }
    }, [language]);
    ...
    const contextValue = useMemo(() => ({
        language,
        setLanguage, // Pass the setter function down
        translations: translations[language]
    }), [language]);

    // Get current translations for footer
    const t = contextValue.translations;

    //Function to navigate AND pass state
    const navigateWithLanguage = (path: string) => {
        navigate(path, { state: { language: language } });
    };
                ...
                <footer className="text-center py-4 md:py-6 bg-red-600 text-white mt-auto">
                    <div className="mb-4 md:mb-5 flex flex-row sm:flex-row justify-center items-center space-y-2 sm:space-y-0 sm:space-x-8 lg:space-x-16">
                        ...
                        <button
                            ...
                            onClick={() => navigateWithLanguage('/termsOfUse')}
                        >
                            {t.terms}
                        </button>
                        <button
                            onClick={() => navigateWithLanguage('/faq')}
                            className="px-4 py-2 sm:px-8 sm:py-3 text-sm text-white font-semibold transition-transform duration-300 hover:scale-110 lg:px-12 lg:py-4 lg:text-lg"
                        >
                            {t.faq}
                        </button>
                        <button
                            className="px-4 py-2 sm:px-8 sm:py-3 text-sm text-white font-semibold transition-transform duration-300 hover:scale-110 lg:px-12 lg:py-4 lg:text-lg"
                            onClick={() => navigateWithLanguage('/privacyPolicy')}
                        >
                            {t.privacy}
                        </button>
                    </div>
